...
export function ProductCard({ product }: ProductCardProps) {
  ...
  // Parse saleUnits if it's a string (SQLite compatibility)
  const rawSaleUnits = productWithUnits.saleUnits;
  const saleUnits: string[] = typeof rawSaleUnits === 'string' 
    ? (rawSaleUnits.includes('[') ? JSON.parse(rawSaleUnits) : rawSaleUnits.split(','))
    : (rawSaleUnits || ['pcs']);

  // Parse pricePerUnit if it's a string (SQLite compatibility)
  const rawPricePerUnit = productWithUnits.pricePerUnit;
  const pricePerUnit: Record<string, number> = typeof rawPricePerUnit === 'string'
    ? (rawPricePerUnit.trim() === '' ? {} : JSON.parse(rawPricePerUnit))
    : (rawPricePerUnit || {});
