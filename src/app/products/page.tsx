...
export default function ProductsPage() {
  ...
  const handleOpenModal = (product?: Product) => {
    if (product) {
      ...
      let pricePerUnit = {};
      try {
        const rawPricePerUnit = (product as any).pricePerUnit;
        pricePerUnit = typeof rawPricePerUnit === 'string' && rawPricePerUnit.trim() !== '' ? JSON.parse(rawPricePerUnit) : (rawPricePerUnit || {});
        if (typeof pricePerUnit !== 'object' || pricePerUnit === null) pricePerUnit = {};
      } catch (e) {
        console.error("Error parsing pricePerUnit", e);
      }
      ...
      setFormData({
        ...
        saleUnits: saleUnits,
        pricePerUnit: Object.keys(pricePerUnit as Record<string, any>).reduce((acc, key) => {
          acc[key] = (pricePerUnit as Record<string, any>)[key]?.toString() || '';
          return acc;
      ...
      >
        ...
        <div className="space-y-4">
          ...
          <div className="border-t border-[var(--border)] pt-4 mt-4">
              ...
              <div className="flex flex-wrap gap-3">
                {units.map((unit) => (
                  <label key={unit.id} className="flex items-center gap-2 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={Array.isArray(formData.saleUnits) && formData.saleUnits.includes(unit.name)}
                      onChange={(e) => {
                        const currentUnits = Array.isArray(formData.saleUnits) ? formData.saleUnits : [];
                        const newUnits = e.target.checked
                          ? [...currentUnits, unit.name]
                          : currentUnits.filter(u => u !== unit.name);
            ...
            {Array.isArray(formData.saleUnits) && formData.saleUnits.length > 1 && (
              <div className="bg-[var(--surface)] rounded-lg p-4">
                <label className="block text-sm font-medium mb-3">Harga per Satuan Jual</label>
                <div className="grid grid-cols-2 gap-3">
                  {(Array.isArray(formData.saleUnits) ? formData.saleUnits : []).filter(u => u !== 'pcs').map((unit) => (
                    <div key={unit}>
